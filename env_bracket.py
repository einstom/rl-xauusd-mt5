"""Gymnasium bracket-order trading environment (H1 decisions, M1 fills).

Reward = three terms (see step(), "reward =" block):
  1. Realized PnL normalized by the per-trade risk budget → +tp_r at TP,
     ≈ -1 at SL, 0 while nothing resolves.  Risk-normalized, never raw cash.
  2. A gentle mark-to-market nudge (reward_mtm_weight, default 0.01) each bar
     an open trade floats in profit (negative while it floats in loss).
  3. A tiny per-bar time penalty (holding_penalty) while a trade is held,
     punishing trades that take too long to resolve.
"""
...
class BracketTradingEnv(gym.Env):
    """One step = one decision bar (H1).  TP/SL detection is simulated inside
    the next interval using M1 execution bars — intrabar precision without
    feeding the agent anything it could not know at decision time.

    Action: MultiDiscrete([3, n_sl_buckets, n_tp_buckets])
        direction: 0 hold/flat (close if open), 1 buy, 2 sell
        sl bucket: ATR multiplier index          (e.g. 1.0x, 1.5x ATR)
        tp bucket: R-multiple index of SL risk   (e.g. 1.5R, 2.0R)

    Position sizing is fixed-fractional (risk_fraction of current equity) —
    the agent controls direction and bracket shape, never the lot size.
    """
    ...
    def __init__(
        ...
        sl_atr_multipliers=(1.0, 1.5),
        tp_r_multipliers=(1.5, 2.0),
        initial_equity: float = 10_000.0,
        risk_fraction: float = 0.005,
        spread_price: float = 0.20,
        slippage_price: float = 0.02,
        commission_per_trade: float = 0.0,
        holding_penalty: float = 0.000002,
        ...
        # sorted DatetimeIndex for O(log n) searchsorted lookups.  This replaces
        # the O(n) boolean-mask scan on every step — with a multi-year M1 file
        # the speedup is ~100-200x per step.
    ...
    def _open_position(self, direction: int, sl_idx: int, tp_idx: int):
        ...
        # Fixed-fractional risk sizing: risk_cash is the budget, sl_dist the
        # stop distance → units follows; the agent never picks size directly.
    ...
    def _simulate_m1_until_next_decision(self) -> float:
        ...
        avoid a full pandas boolean-mask scan on each step.
        """
        p = self.position
        if p.direction == 0:
            return 0.0
    ...
    def step(self, action):
        ...
        # Dense mark-to-market shaping term after the interval.
        # Uses the CURRENT bar's close (known at decision time) to avoid
        # lookahead: the trade stays open, so no future price leaks in.
        if self.position.direction != 0:
        ...
        # ── Reward: the three terms ─────────────────────────────────────────
        # Term 1 — realized PnL / risk budget: +tp_r on TP, ≈ -1 on SL, 0 idle.
        reward = (self.equity - prev_equity) / reward_risk_unit
        if self.position.direction != 0:
            # Term 2 — gentle unrealized-PnL nudge while the trade floats.
            reward += (unrealized / max(self.position.risk_cash, 1e-12)) * self.reward_mtm_weight
            # Term 3 — tiny per-bar time penalty while the trade is held.
